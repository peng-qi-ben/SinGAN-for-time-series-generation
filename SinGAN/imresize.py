...
def contributions(in_length, out_length, scale, kernel, kernel_width, antialiasing):
    ...
    # field_of_view = (np.uint(left_boundary))
    # Assign weight to each pixel in the field of view. A matrix whos horizontal dim is the output pixels and the
    # vertical dim is a list of weights matching to the pixel in the field of view (that are specified in
    # 'field_of_view')
    weights = fixed_kernel(1.0 * np.expand_dims(match_coordinates, axis=1) - field_of_view - 1)
    # weights = fixed_kernel(1.0 * match_coordinates- field_of_view - 1)
    # Normalize weights to sum up to 1. be careful from dividing by 0
    sum_weights = np.sum(weights)
    # sum_weights[sum_weights == 0] = 1.0
    # weights = 1.0 * weights / np.expand_dims(sum_weights)
    weights = 1.0 * weights/sum_weights
    ...
    non_zero_out_pixels = non_zero_out_pixels[0]
    weights = weights[:, non_zero_out_pixels]
    # weights = np.squeeze(weights[:, non_zero_out_pixels])
    field_of_view = np.squeeze(field_of_view[:, non_zero_out_pixels])
    # field_of_view = field_of_view[:, non_zero_out_pixels]
    ...
    try:
        ...
        field_of_view = np.squeeze(field_of_view)
    except:
        pass
...
def resize_along_dim(im, dim, weights, field_of_view):
    ...
    # same number
    a = field_of_view.T
    b = tmp_im[field_of_view.T]
    c = tmp_im[field_of_view.T] * weights
